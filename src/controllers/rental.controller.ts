...
export const getAllRentals = catchAsyncErrorWithCode(
  async (req: QueryRequest<{}>, res: Response) => {
    const rentals = await service.getAllRentals();
    res
      .status(200)
      .json(
        ResponseHelper.success(rentals, "Lấy danh sách hóa đơn thuê thành công")
      );
    return;
  },
  "GET_ALL_RENTALS_ERROR"
);
